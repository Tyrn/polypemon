...
Try **Polypemon**, this way:

::

    Robinson Crusoe $ polypemon -via 'Daniel Defoe' -m 'Robinson Crusoe' . ~/MyAudioLibrary
...
::
    ...
    Robinson Crusoe $ polypemon -a 'Daniel Defoe' -u 'Robinson Crusoe' . ~/MyAudioLibrary

Notice the tags set by **Polypemon**.

Description
===========

**Polypemon** is a CLI utility for basic processing and copying
...
order. **Polypemon** tries hard to sort the tracks properly.
To check the track order visually use ``-v`` or ``-vi``, and avoid ``-u``.

**Polypemon** renames directories and audio files, replacing tags,
...
::
    ...
    $ polypemon [<options>] <source directory> <destination directory>
...
::
    ...
    Source Album $ polypemon -dst . /run/media/user/F8950/Audiobooks/
